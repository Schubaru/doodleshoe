...
- **Monkey astronaut** — the companion, and the site's scroll cue. Until the first scroll he lives below the fold at bottom-centre, swimming up into view and sinking away on a ~7.8s cycle — "there is more down here", played by the cast instead of a chevron. The first scroll promotes him, permanently, to his post in the right lane (one 1.5s eased arc, cresting ~44px above the straight line so it reads as swimming); scrolling back to the top does not demote him. `position: fixed` in the reserved right lane (`--gutter-x`, 148px, collapsing to 0 below 900px where he is hidden). Three motion layers compose so they never fight: the wrapper carries the journey (`--mx` / `--my`, JS rAF) and the scroll spring (`--drift` / `--lean` — gated off until he is home), the inner SVG carries a lazy 14s figure-eight orbit, and one arm carries the wave. The peek only arms when JS runs, motion is allowed, the load is at scrollY 0 and the viewport is ≥900px — every other visitor (no-JS, reduced motion, mobile, mid-page refresh, `?p`/`?solo`/`?shift` screenshot rigs) gets him parked at home, because the CSS vars default to 0. He flips to white over the Arbiter's black page.
...
- **Taped screenshot**: real screenshot, slight rotation, masking-tape corner SVGs. Three arrangements, one per case, and they must stay distinct — two identical fans back to back read as a template rather than a hand. `.phone-shot` is Wordsword's dealt deck (each screen steps right and down); `.stack-shot` is Unscary.ai's centre-forward trio (the middle screen leads, the wings open out from behind it); `.wide-shot` is one letterboxed dashboard with two close-ups taped over it.
