...
- Single long-scroll page. Robin-noguier-style case studies: each study is a tall section with a sticky 100vh takeover panel; internal scroll progress advances beats (what → how it works → how it was built).
...
- **Case-study panel**: paper card, ink border, offset shadow; header row (Shantell title + hand-drawn platform badge), beats as internal scroll steps, footer "built with" mono line.
...
- **Arbiter chat bubble**: mini chat with one tappable prompt → "Were it so easy." Easter egg.
